...
- **담당 역할**: 
  - 프론트엔드 UI/UX 구현 (React SPA 및 3D 시뮬레이션 인터페이스)
  - AI 추천 및 대화형 백엔드 엔진 개발 (FastAPI)
  - 데이터베이스 설계, 수집 파이프라인 및 Supabase 연동
  - 3D 실내 맵 렌더링 및 로봇 관제(ROSBridge) 웹 소켓 통신 구현
...
| 이름 | 역할 |
|---|---|
| **본인 (1인 개발 및 통합)** | 프론트엔드 UI/UX 구현, FastAPI 백엔드 개발, Hybrid Recommender 모델 및 Paige 오케스트레이터 설계, 3D 실내 맵 및 ROSBridge 통신 제어 |
